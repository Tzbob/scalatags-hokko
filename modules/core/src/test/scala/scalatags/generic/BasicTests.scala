...
class BasicTests[Builder, Output <: FragT, FragT](
  ...
  val tests = TestSuite {
    ...
    'compileErrors {
      'niceErrorsForAttributes {
        ...
        assert(
          msg.contains(
            "scalatags does not know how to use () => String as an attribute"))
      }
      'niceErrorsForStyles {
        val msg = compileError("""a(opacity:= {() => "lol"})""").msg
        assert(
          msg.contains(
            "scalatags does not know how to use () => String as an style"))
      }
    }
    'nulls {
    ...
    // TODO: Tests errors in new jsdom
//    'rawAttrs-{
//      strCheck(
//        button(
